      ...
      <div className="mx-auto max-w-3xl p-6">
        <Card title="No Profile Selected">
          <p className="text-text-secondary mb-4">
            Create a birth profile to view fixed star conjunctions.
          </p>
    ...
    <div className="mx-auto max-w-7xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-text-primary">
          Fixed Stars for {activeProfile.name}
        </h1>
        <p className="mt-1 text-sm text-text-muted">
          Major fixed star conjunctions with natal planets
        </p>
      </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border text-left">
                      ...
                      <th className="pb-2 font-medium text-text-muted">Nature</th>
                    </tr>
                  </thead>
                  <tbody>
                    {conjunctions.map((c, i) => (
                      <tr key={i} className="border-b border-border/50 last:border-0">
                        ...
                        <td className="py-2.5 pr-4 font-mono text-text-secondary">
                          {typeof c.orb === "number" ? `${c.orb.toFixed(2)}\u00b0` : String(c.orb ?? "")}
                        </td>
                        <td className="py-2.5 pr-4 text-text-secondary">
                          {String(c.magnitude ?? "")}
                        </td>
                        <td className="py-2.5">
                          <Badge variant="info">{String(c.nature ?? c.influence ?? "")}</Badge>
                        </td>
                      </tr>
                    ))}
                  </tbody>
